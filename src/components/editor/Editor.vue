...
import { useDialogueStore, useEditorDraftStore, useTalkStore } from '@/store'
...
import MeEditorRecord from './MeEditorRecord.vue'
import MeEditorRecorder from './MeEditorRecorder.vue'
import MeEditorRhetoric from './MeEditorRhetoric.vue'
import MeEditorMaterial from './MeEditorMaterial.vue'
...
const isShowHS = ref(false)
const isShowEditorRecorder = ref(false)
const isShowEditorRecord = ref(false)
  ...
  modules: {
    ...
    mention: {
      ...
      renderItem: (data: any) => {
        ...
        el.innerHTML = `<img src="${data.avatar?data.avatar:'/src/assets/image/notify.png'}" class="avator"/>`
...
const navsR = reactive([
  {
    title: '常用语',
    icon: markRaw(FileTxt),
    show: true,
    click: () => {
      talkStore.showRhetoricModal(true)
    }
  },
  {
    title: '资料',
    icon: markRaw(FileAddition),
    show: true,
    click: () => {
      talkStore.showMaterialModal(true)
    }
  },
])
...
function onRecordEvent(data: any) {
  const msg = emitCall('record_event', data, (_: boolean) => {
    isShowEditorRecord.value = false
  })
...
onMounted(() => {
  // isShowHS.value = storage.get('IsStaff')
  isShowHS.value = true
...
<template>
  ...
  <MeEditorRecord
  v-if="isShowEditorRecord"
  @on-submit="onRecordEvent"
  @close="isShowEditorRecord = false"
/>
  ...
  />
  <MeEditorRhetoric @on-submit="onRecordEvent" />
  <MeEditorMaterial @on-submit="onRecordEvent" />
</template>
