...
const emit = defineEmits(['close', 'to-talk', 'label'])
...
const editLabelPopover = ref(false)
...
function loadDetail() {
  ServeGroupDetail({
    ...
    if (res.code == 200) {
      ...
      state.detail.group_label = result.group_label
      state.label = result.group_label
...
const onChangeGroupLabel = () => {
  ServeUpdateGroupLabel({
    group_id: props.gid,
    group_label: state.label
  }).then(({ code, message }) => {
    if (code == 200) {
      // @ts-ignore
      editLabelPopover.value.setShow(false)
      state.detail.group_label = state.label
      talkStore.loadTalkList()
      dialogueStore.setTalk({ ...dialogueStore.talk, username: state.label ? state.label : state.detail.name })
      window['$message'].success('已更新群备注')
      loadMembers()
    } else {
      window['$message'].error(message)
    }
  ...
  <section class="el-container is-vertical section">
    ...
    <main class="el-main main me-scrollbar me-scrollbar-thumb">
      <div class="info-box">
        ...
        <div class="b-box">
          <div class="block">
            <div class="title">群备注：</div>
            <div class="text">
              <n-popover trigger="click" placement="left" ref="editLabelPopover">
                <template #trigger>
                  <n-button type="primary" text> 设置 </n-button>
                </template>

                <template #header> 设置我的群备注 </template>

                <div style="display: flex">
                  <n-input
                    type="text"
                    placeholder="群备注，清空恢复群名称"
                    maxlength="40"
                    clearable
                    v-model:value="state.label"
                    @keydown.enter="onChangeGroupLabel"
                  />
                  <n-button type="primary" class="mt-l5" @click="onChangeGroupLabel">
                    确定
                  </n-button>
                </div>
              </n-popover>
          ...
          <div class="describe">{{ state.detail.group_label || '未设置' }}</div>
        </div>
