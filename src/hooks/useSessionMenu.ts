...
import { ServeTopTalkList, ServeDeleteTalkList, ServeSetNotDisturb, ServeMarkTalkUnreadNum } from '@/api/chat'
...
export function useSessionMenu() {
  ...
  const onContextMenu = (e: any, item: ISession) => {
    ...
    if (item.talk_type == 2) {
      options.push({
        icon: renderIcon(AddOne),
        label: '添加分组',
        key: 'add_group'
      })
      options.push({
        icon: renderIcon(Tips),
        label: '案件跟进',
        key: 'follow'
      })
    }
  ...
  const onAddGroup = (item: ISession) => {
    talkStore.setGroupModalItem(item)
    talkStore.showGroupModal(true)
  }

  const onFollow = (item: ISession) => {
    // 使用正确的事件总线
    bus.emit('follow:show', item)
  }
  ...
  // 设置消息免打扰
  const onMark = (item: ISession) => {
    ServeMarkTalkUnreadNum({
      ...
      if (code == 200) {
        ...
        talkStore.updateItem({
          ...
          unread_num: item.unread_num > 0 ? item.unread_num : 1
        })
      } else {
