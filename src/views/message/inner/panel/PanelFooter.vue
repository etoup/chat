...
const onSendVideoEvent = async ({ data }) => {
  ...
  uploadsStore.initUploadFile(data, props.talk_type, props.receiver_id, dialogueStore.talk.username, cover.data.src)
...
const onSendFileEvent = ({ data }) => {
  ...
  if (data.size > maxsize) {
    return window['$message'].warning('上传文件不能超过200M!')
  }
...
const onSendRecordEvent = async ({ data, callBack }) => {
  console.log('onSendRecordEvent', data)

  talkStore.isShowRhetoricModal = false
  talkStore.isShowMaterialModal = false

  onSendMessage({ type: 'text', content: data.content, files: data.files, id: data.id }, callBack)
}
...
const evnets = {
  ...
  record_event: onSendRecordEvent,
