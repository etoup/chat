...
import { NDropdown, NCheckbox, NModal, NCard, NSpace, NForm, NFormItem, NCheckboxGroup, NInput, NButton, NSelect } from 'naive-ui'
import { Loading, MoreThree, ToTop } from '@icon-park/vue-next'
import { bus } from '@/utils/event-bus'
import { useDialogueStore, useTalkStore } from '@/store'
import { formatTime, parseTime, parseDate } from '@/utils/datetime'
...
const isShowTalkTime = (index: number, datetime: string) => {
  ...
  if (records.value[index].is_revoke == 1) return false
  const out = parseDate(datetime)
  const curr = Math.floor(Date.now() / 1000)
  const sec = Math.floor(out.getTime() / 1000)
  if (curr - sec < 300) return false
  if (index == records.value.length - 1) return true
  const nextOut = parseDate(records.value[index + 1].created_at)
  return parseTime(out, '{y}-{m}-{d} {h}:{i}') != parseTime(nextOut, '{y}-{m}-{d} {h}:{i}')
}
...
// 语音转译
const onVoiceTranslate = (data: ITalkRecord) => {
  dialogueStore.ApiVoiceTranslate(data.extra.url, data.msg_id)
}
...
const onGather = (data: ITalkRecord) => {
  console.log('onGather', data)
  show.value = true
  item.value = data
}
...
const handleSubmit = () => {
  formRef.value.validate(async (errors: any) => {
    if (!errors) {
      const res = await ServeCollectHandle({keys: [item.value.msg_id], ...model.value})
      if (res.code == 200) {
        useMessage.success('收集成功')
        model.value = {
          type: null,
          remark: '',
          progress_item_id: null
      ...
      <n-space vertical>
        <n-form ref="formRef" :model="model" :rules="rules">
          <n-form-item label="类型" path="type">
            <n-checkbox-group  v-model:value="model.type">
              <n-space item-style="display: flex;">
                <n-checkbox value="1" label="转资料"  />
                <n-checkbox value="2" label="转进度" />
              </n-space>
            </n-checkbox-group>
          </n-form-item>
          <n-form-item label="备注" path="remark">
            <n-input v-model:value="model.remark" placeholder="输入备注信息" />
          </n-form-item>
          <n-form-item label="进度" path="progress_item_id">
            <n-select
              v-model:value="model.progress_item_id"
              :options="options"
              placeholder="请选择进度"
              style="width: 100%"
            />
