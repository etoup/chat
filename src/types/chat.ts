...
export interface ISession {
  ...
  case_status: number // 案件状态
  case_progress: number // 案件进度
  unread_num: number // 未读消息数
  created_at: string // 创建发送时间
  updated_at: string // 最后发送时间
  content?: string // 消息内容
  last_message?: ISessionLastMessage
  draft_text?: string // 草稿文本
  is_v2?: boolean
  msg_text: String // 消息文本
  enterprise_case_id: number // 企业案件ID
  total_messages: number
  service_ratio: number // 服务比率
  service_amount: string // 服务金额
}
...
export interface ITalkRecordExtraMention {
  name: string
  content: string
  remark: string
  datetime: string
