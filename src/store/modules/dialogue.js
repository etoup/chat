...
export const useDialogueStore = defineStore('dialogue', {
  state: () => {
    return {
      ...
      talk: {
        ...
        receiver_id: 0,
        case_status: 0 // 案件状态[0:正常;10:已结案]
      },
  ...
  actions: {
    ...
    setDialogue(data = {}) {
      ...
      this.talk = {
        username: data.remark || data.title || data.name,
    ...
    // 语音聊天记录转译
    ApiVoiceTranslate(url, msg_id = '') {
      ServeVoiceTranslate({ url }).then((res) => {
        if (res.code == 200) {
          const {result} = res.data
          this.updateDialogueRecord({ msg_id, result })
        } else {
          window['$message'].warning(res.message)
        }
      })
